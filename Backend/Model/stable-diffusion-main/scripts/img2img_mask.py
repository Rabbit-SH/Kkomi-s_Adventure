...
def load_model_and_preprocess_image(image_path):
    ...
    human_mask = predicted_class == 15
    human_mask_bool = human_mask.astype(bool)
    input_image_np = np.array(input_image)
    human_only = np.zeros_like(input_image_np)
    human_only[human_mask_bool] = input_image_np[human_mask_bool]
    background = input_image_np.copy()
    background[human_mask_bool] = 0
...
def main():
    ...
    # 사람과 배경 분리
    human_image, background_image = load_model_and_preprocess_image(opt.init_img)
    # 원본 이미지를 텐서로 변환
